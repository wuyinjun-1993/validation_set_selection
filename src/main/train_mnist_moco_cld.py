...
from clustering_method.NN import *
...
def build_model(args):
    ...
    model = DNN_three_layers(args.nce_k, low_dim=args.low_dim).cuda()
    model_ema = DNN_three_layers(args.nce_k, low_dim=args.low_dim).cuda()
...
def main(args):
    ...
    # tensorboard
    if dist.get_rank() == 0:
        summary_writer = SummaryWriter(log_dir=args.save_path)
    else:
        summary_writer = None
