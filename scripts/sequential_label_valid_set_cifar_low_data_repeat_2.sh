...
add_valid_in_training_flag="--cluster_method_three --cosin_dist --weight_by_norm --not_rescale_features --replace --use_model_prov --model_prov_period 20 --total_valid_sample_count ${total_valid_sample_count} --all_layer_grad_no_full_loss"
...
exe_cmd="python -m torch.distributed.launch \
  ...
  --low_data \
  --low_data_num_samples_per_class ${err_label_ratio} \
  --save_path ${save_path_prefix}_valid_select_0_3/ \
...
output_file_name=${output_dir}/output_${dataset_name}_low_data_${err_label_ratio}_valid_select_0_3.txt
...
do
    ...
    --low_data \
    --low_data_num_samples_per_class ${err_label_ratio} \
    --save_path ${save_path_prefix}_valid_select_${k}_3/ \
    --prev_save_path ${save_path_prefix}_valid_select_$(( k - 1 ))_3/ \
    --cuda \
    --lr ${lr} \
    --batch_size ${batch_size} \
    --test_batch_size ${test_batch_size} \
	...
	output_file_name=${output_dir}/output_${dataset_name}_low_data_${err_label_ratio}_valid_select_seq_select_${k}_3.txt
