...
exe_cmd="python -m torch.distributed.launch \
  ...
  --flip_labels \
  --err_label_ratio ${err_label_ratio} \
  --save_path ${save_path_prefix}_uncertain_select_0/ \
...
output_file_name=${output_dir}/output_${dataset_name}_biased_error_${err_label_ratio}_uncertain_select_0.txt
...
do
    ...
    --flip_labels \
    --err_label_ratio ${err_label_ratio} \
    --save_path ${save_path_prefix}_uncertain_select_$k/ \
    --prev_save_path ${save_path_prefix}_uncertain_select_$(( k - 1 ))/ \
    --cuda \
    --lr ${lr} \
    --batch_size ${batch_size} \
    --test_batch_size ${test_batch_size} \
	...
	output_file_name=${output_dir}/output_${dataset_name}_biased_error_${err_label_ratio}_uncertain_select_$k.txt
